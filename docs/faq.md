...
本仓库负责保存知识和规则。真实业务代码应在具体影刀应用的项目目录中修改。
...
复杂任务先给简短计划，再按步骤修改和验证即可，不为一次开发过程创建临时进度文件。

需要长期保留的业务规则、待确认事项或设计决策，应写入对应 PRD、README、ADR 或其他正式项目文档，避免临时进度文件过期后与真实代码和业务文档冲突。
