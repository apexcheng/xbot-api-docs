...
5. `shadowbot_sync_tool.py` 位于真实项目根目录。新增文件后执行 `python shadowbot_sync_tool.py prepare`。
6. 用户说“同步影刀”“执行影刀同步”“同步到影刀”等同类表述时，均指执行 `python shadowbot_sync_tool.py prepare`。
