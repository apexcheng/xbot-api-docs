...
- 市场指令文档只写参数名、没写真实可选值时，不要根据可视化界面中文选项猜编码版参数。
- 可视化流程能运行，不代表编码版参数可以传中文或展示值；如果源码判断 `close_type == "click"`，编码版就传 `"click"`，不是 `"点击"`。
- `__init__.py` 可能只是包装层，不一定包含真实业务逻辑；确认参数值、默认值、分支判断和返回结构时，应继续查看 `_core.py`、`core.py` 或具体业务 `.py`。
- 不要在业务代码里反复试多个候选值做兜底；先用 `inspect.signature()`、`inspect.getfile()` 和源码判断确认真实枚举，再改业务代码。
- 不要预设所有市场指令都返回 `dict`、`list` 或固定对象；封装某个指令前，先通过文档、源码或运行结果确认返回结构。
- 钉钉 AI 表格指令按 `data.records` 取记录，是该指令的已验证约定，不应泛化到所有市场指令。
- 本仓库只记录排查方法、参数结论和调用示例，不要提交影刀运行包源码、反编译源码、第三方市场指令完整源码、账号、token 或 Cookie。
- 已确认且稳定的 API 或参数说明，回写到 `xbot-api-docs/docs/`；尚未验证的参数、返回结构或行为差异，标注“需运行验证”。
