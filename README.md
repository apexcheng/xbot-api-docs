...
- `TASK.md` 工作流：仅用于需要跨会话持续推进、阶段很多或待确认事项较多的任务
...
读取项目根目录 AGENTS.md，并确认固定知识库路径存在
          ↓
仅在需要跨会话持续推进、阶段很多或待确认事项较多时创建 TASK.md
          ↓
检查真实项目现有代码
          ↓
...
`TASK.md` 不是复杂任务的默认要求。适合创建 `TASK.md` 的情况：

- 任务需要跨会话持续推进
- 阶段较多，需要把进度持久化给后续 Agent
- 存在较多待确认事项或长期未完成事项
- 用户明确要求用任务文件记录实施过程

普通多文件修改、一次会话内可以完成的复杂任务，只需先给简短计划，不必额外创建任务文件；简单修改则直接做最小修改。
